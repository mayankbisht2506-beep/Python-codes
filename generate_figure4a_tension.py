...
if len(data_flat) == N_FULL * N_FULL + 1:
    ...
    print(f"Error: Matrix size mismatch.")
    exit()
...
# ==========================================
# 4. PHYSICS MODELS (QUADRUPLE CONCORDANCE)
# ==========================================
C_LIGHT = 299792.458
...
# UPDATED PARAMETERS for H0 = 73.74
# Shift = -5 * log10(73.74 / 67.4) = -0.195
MAG_SHIFT = -0.195 
Z_TRANS = 0.65
WIDTH = 0.15       # Standardized width
...
# Sigmoid: 1 at Low z (Late), 0 at High z (Early)
# Physics: The Late Universe is 'Stiff', H0 rises to 73.74, making SNe brighter (lower mu).
...
print("Target:   SH0ES / Vacuum Model (H0=73.74)")
print("="*50)
...
# Plot Viscous Prediction Curve (Fixed Sign Error)
z_sort = np.sort(df_clean['zHD'])
# Corrected Sigmoid for Plot: Matches Model B logic above
...
plt.savefig("Tension_Resolution_Result.png")
plt.show()
